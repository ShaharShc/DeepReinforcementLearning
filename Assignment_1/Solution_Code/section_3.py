...
from keras.layers import Input, Flatten, Dense, BatchNormalization, Input, Lambda, Add
...
def generate_model(observation_space, action_space, n_hidden_layers):
    ...
    X = Dense(64, input_shape=(observation_space,), activation="relu", kernel_initializer=init)(X_Input)
    X = Dense(32, activation="relu", kernel_initializer=init)(X)
    X = Dense(16, activation="relu", kernel_initializer=init)(X)

    #dueling
    state_value = Dense(1, kernel_initializer=init)(X)
    state_value = Lambda(lambda s: K.expand_dims(s[:, 0], -1), output_shape=(action_space,))(state_value)

    action_advantage = Dense(action_space, kernel_initializer=init)(X)
...
def experience_replay(replays, model, target_model, observation_space, loss_lst, total_steps):
    ...
    sample_batch = random.sample(replays, BATCH_SIZE) # sample a minibatch randomly from the experience_replay.
...
def train_agent(env, observation_space, action_space, n_hidden_layers):
    ...
    for episode in range(EPISODES):
        ...
        for step in range(STEPS):
            ...
            model, target_model, loss_lst = experience_replay(replays, model, target_model, observation_space, loss_lst, overall_steps)
            ...
            if terminated:
                ...
                print("Episode:", episode + 1, "/", EPISODES, "Epsilon:", epsilon, "Learning rate:", learning_rate, "Steps:", step + 1, "Score", total_rewards, "Avg over 100:", avg_reward_over_100_ep)
                if avg_reward_over_100_ep >= 475:
                    episodes_till_475.append(episode + 1)
                    print("######################### got avg reward of " + str(avg_reward_over_100_ep) + " over 100 cosecutive episodes in episode " + str(episode + 1) + " #########################")
